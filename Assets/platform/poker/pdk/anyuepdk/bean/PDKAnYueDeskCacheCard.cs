...
{
    public class PDKAnYueDeskCacheCard : BytesSerializable
    {
        public int[] states;

        public PDKAnYueCardInfo[] deskCacheCard;

        public PDKAnYueDeskCacheCard(int len)
        {
            states = new int[len];
            deskCacheCard = new PDKAnYueCardInfo[len];
            for (int i = 0; i < len; i++)
            {
                states[i] = 0;
                deskCacheCard[i] = new PDKAnYueCardInfo(i);
                deskCacheCard[i].cards = new int[0];
            }
        }

        public void clone(PDKAnYueDeskCacheCard bean)
        {
            for (int i = 0; i < bean.states.Length; i++)
            {
                states[i] = bean.states[i];
                deskCacheCard[i] = (PDKAnYueCardInfo)bean.deskCacheCard[i].clone();
            }
        }

        /// <summary> 添加桌面上的牌 </summary>
        public void add(PDKAnYueCardInfo info, bool sound = true)
        {
            ...
            if (deskCacheCard[info.master].cards.Length > 0)//出牌
            {
                ...
                PDKAnYueMatch.match.tips.reset(deskCacheCard[info.master], PDKAnYueMatch.match.getSelfHandCard());
        ...
        public void addReplay(PDKAnYueCardInfo info, bool sound = true)
        {
            deskCacheCard[info.master] = info;
        ...
        {
            states[master] = 0;
            deskCacheCard[master] = new PDKAnYueCardInfo(0);
        ...
        public PDKAnYueCardInfo getCardsInfo(int master)
        {
            return deskCacheCard[master];
        }

        /// <summary> 比较玩家选择的牌与其他玩家牌大小 </summary>
        public bool compareTo(PDKAnYueCardInfo userCards)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public PDKAnYueCardInfo getLastCard()
        {
            int last = Room.room.indexOf();
            int mine = last;
            for (int i = 0; i < Room.room.getPlayerCount() - 1; i++)
