...
namespace platform.spotred.room
{
    ...
    /// 接收招吃消息
    /// </summary>
    public class RecvZhaoChowCardsProcess:Process
    {
        int operate;//操作类型

        private ZhaoChowOperate op;

        public RecvZhaoChowCardsProcess(ZhaoChowOperate op)
        {
            this.op = op;
        ...
        public override void execute()
        {
            ...
            panel.showCard(op.index, op.card,SendView.ZHAOCHOW);
            panel.setOperate(operate);
            CPMatch.match.step = op.step;
            CPMatch.match.setStage(op.stage);

            SoundManager.playZhaoChi(Room.room.players[op.index].player.sex);
