...
namespace platform.mahjong
{
    ...
    /// 杠牌
    /// </summary>
    public class MJMatchKongProcess : Process
    {
        MJMatchKongOperate operate;

        int selfOperate;

        public MJMatchKongProcess(MJBaseOperate operate)
        {
        ...
        public override void execute()
        {
            ...
            {
                panel.refreshHandCard(operate.index, match.getSelfTingCards(MJMatch.match.mindex),false);
            }
            else
            {
        ...
        private void showOperate()
        {
            if (selfOperate > 0)
            {
                panel.gameView.getOperateView().showOperate(MJMatch.match.getMJOperateInfos(selfOperate, operate.card, true, MJMatch.match.mindex));
            }
            operate.playOver();
        }
    }
