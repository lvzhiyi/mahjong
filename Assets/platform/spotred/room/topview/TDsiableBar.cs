...
{
    public class TDsiableBar: UnrealLuaSpaceObject
    {
        ...
        protected override void xrefresh()
        {
            ...
            float x = (float)(Math.Sin(angles) * dis);
            float y = -(float)Math.Sqrt(Math.Pow(dis, 2) - Math.Pow(x, 2));
            x -= ((index_space / line) * 43);
