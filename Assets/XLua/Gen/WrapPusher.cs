...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderUnityEngineVector2
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<System.IO.Compression.CompressionMode>(translator.PushSystemIOCompressionCompressionMode, translator.Get, translator.UpdateSystemIOCompressionCompressionMode);
        ...
        int SystemIOCompressionCompressionMode_TypeID = -1;
		int SystemIOCompressionCompressionMode_EnumRef = -1;
        
        public void PushSystemIOCompressionCompressionMode(RealStatePtr L, System.IO.Compression.CompressionMode val)
        {
            if (SystemIOCompressionCompressionMode_TypeID == -1)
            {
			    bool is_first;
                SystemIOCompressionCompressionMode_TypeID = getTypeId(L, typeof(System.IO.Compression.CompressionMode), out is_first);
				
				if (SystemIOCompressionCompressionMode_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(System.IO.Compression.CompressionMode));
				    SystemIOCompressionCompressionMode_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, SystemIOCompressionCompressionMode_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, SystemIOCompressionCompressionMode_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for System.IO.Compression.CompressionMode ,value="+val);
            }
			
			LuaAPI.lua_getref(L, SystemIOCompressionCompressionMode_EnumRef);
        ...
        public void Get(RealStatePtr L, int index, out System.IO.Compression.CompressionMode val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != SystemIOCompressionCompressionMode_TypeID)
				{
				    throw new Exception("invalid userdata for System.IO.Compression.CompressionMode");
				}
                ...
                {
                    throw new Exception("unpack fail for System.IO.Compression.CompressionMode");
                }
				val = (System.IO.Compression.CompressionMode)e;
                
            }
            else
            {
                val = (System.IO.Compression.CompressionMode)objectCasters.GetCaster(typeof(System.IO.Compression.CompressionMode))(L, index, null);
            }
        }
		
        public void UpdateSystemIOCompressionCompressionMode(RealStatePtr L, int index, System.IO.Compression.CompressionMode val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for System.IO.Compression.CompressionMode ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(System.IO.Compression.CompressionMode[]))
			{
			    System.IO.Compression.CompressionMode[] array = obj as System.IO.Compression.CompressionMode[];
				translator.PushSystemIOCompressionCompressionMode(L, array[index]);
				return true;
			}
