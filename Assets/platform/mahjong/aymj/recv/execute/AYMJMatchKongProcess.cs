...
namespace platform.mahjong
{
    ...
    /// 杠牌
    /// </summary>
    public class AYMJMatchKongProcess : Process
    {
        MJMatchKongOperate operate;

        int selfOperate;

        public AYMJMatchKongProcess(MJBaseOperate operate)
        {
        ...
        private void showOperate()
        {
            if (selfOperate > 0)
            {
                panel.gameView.getOperateView().showOperate(AYMJMatch.match.getMJOperateInfos(selfOperate, operate.card, true, AYMJMatch.match.mindex));
            }
            operate.playOver();
        }
    }
