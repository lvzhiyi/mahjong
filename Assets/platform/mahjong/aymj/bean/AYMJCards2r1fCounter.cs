...
namespace platform.mahjong
{
    ...
    /// 牌型计算器
    /// </summary>
    public class AYMJCards2r1fCounter : AYMJCardsCounter
    {
        /// <summary></summary>
        public AYMJCards2r1fCounter(Rule rule, ArrayList<int> cards, ArrayList<FixedCards> fixs, int paidui):base(rule, cards, fixs,paidui)
        {
            
        }

        /// <summary> 验证是否可以胡牌 </summary>
        public override AYMJSettle getHuSettle(int tile, int huType, ArrayList<FixedCards> fixeds)
        {
        ...
        public override AYMJSettle getHuAcco(int huType,int huCard, ArrayList<FixedCards> fixeds)
        {
            // 所有牌型都是按照先检查大牌型的顺序
        ...
        /// 获取牌型的番数
        /// </summary>
        protected override void pTypePoint(int root, int bits, ref int atype, ref int ptype)
        {
            if (isCommon(bits))
            {
                if (isWith19())
        ...
        /// 额外加番项
        /// </summary>
        protected override void aTypePoint(int bits, ref int atype, ArrayList<FixedCards> fixeds)
        {
            if (isNo19()) // 中张（断幺九）
            {
